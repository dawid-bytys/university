...
int main(int argc, const char *argv[]) {
  ...
  // Convert arguments
  int processes_count = atoi(argv[3]);
  int critical_sections_count = atoi(argv[4]);

  // Open a file to store semaphore state
  int storage = open(argv[2], O_RDWR | O_CREAT | O_TRUNC, 0777);
  if (storage == -1) {
  ...
  printf("The new semaphore's address: %p", (void *)new_semaphore);
  ...
  for (int i = 0; i < processes_count; i++) {
    ...
    if (process == -1) {
      ...
      if (execl(concat("./", argv[1]), argv[1], argv[2], name,
                critical_sections_count, NULL) == -1) {
  ...
  // Verify the states
  state_verification(argv[1], processes_count, critical_sections_count);
